...
//Usage: webget urlfn main() {
    let mut url: String = std::env::args().last().unwrap();
    //got all my basic variables; Don't know what's next    //parse to get message parts; skip over the https:// in url    let mut parts: Vec<&str> = url.split("/").collect();
    let mut host: String = parts[3].to_string();
    let mut req = "".to_string();
    for entry in parts.iter() {
        req += "/{entry}";
    }
    send_message(host.as_str(), 443, req.as_str()); 
...
fn send_message(host: &str, port: usize, message: &str) -> io::Result<()> {
    ...
    //create buff reader on the stream     let buf = BufReader::new(stream);
